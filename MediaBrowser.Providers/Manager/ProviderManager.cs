...
namespace MediaBrowser.Providers.Manager
{
    ...
    public class ProviderManager : IProviderManager, IDisposable
    {
        ...
        private async Task SaveMetadataAsync(BaseItem item, ItemUpdateType updateType, IEnumerable<IMetadataSaver> savers)
        {
            ...
            await Task.WhenAll(applicableSavers.Select(async saver =>
            {
            ...
            where TLookupType : ItemLookupInfo
        {
            ...
            // Fan-out: query all providers in parallel, collect results arrays indexed by provider.
            var providerResults = await Task.WhenAll(providerList.Select(async provider =>
            {
                try
                {
                    var results = (await provider.GetSearchResults(searchInfo.SearchInfo, cancellationToken).ConfigureAwait(false)).ToList();
...
#pragma warning restore CA1031 // do not catch general exception types
            ...
            // Deduplicate serially after all providers have responded.
