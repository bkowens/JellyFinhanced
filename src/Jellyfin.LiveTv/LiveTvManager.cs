...
namespace Jellyfin.LiveTv
{
    ...
    public class LiveTvManager : ILiveTvManager
    {
        ...
        public Task AddInfoToProgramDto(IReadOnlyCollection<(BaseItem Item, BaseItemDto ItemDto)> programs, IReadOnlyList<ItemFields> fields, User user = null)
        {
            ...
            // Issue #6 fix: batch-load all distinct channels in one query instead of N individual GetItemById calls.
            Dictionary<Guid, LiveTvChannel> channelMap = null;
            if (hasChannelInfo || hasChannelImage)
            {
                var distinctChannelIds = programs
                    .Select(p => ((LiveTvProgram)p.Item).ChannelId)
                ...
                {
                    channelMap = _libraryManager.GetItemList(new InternalItemsQuery
                    {
                        ItemIds = distinctChannelIds,
                        DtoOptions = new DtoOptions(false) { EnableImages = hasChannelImage }
                    })
            ...
            foreach (var (item, dto) in programs)
            {
                ...
                if ((hasChannelInfo || hasChannelImage)
                    && channelMap is not null
                    && channelMap.TryGetValue(program.ChannelId, out var liveChannel))
                {
        ...
        public async Task<QueryResult<TimerInfoDto>> GetTimers(TimerQuery query, CancellationToken cancellationToken)
        {
            ...
            foreach (var i in timers)
            {
                ...
                returnList.Add(await _tvDtoService.GetTimerInfoDto(i.Item1, i.Item2, program, channel).ConfigureAwait(false));
            }
