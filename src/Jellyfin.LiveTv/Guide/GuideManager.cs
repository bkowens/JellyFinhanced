...
public class GuideManager : IGuideManager
{
    ...
    private async Task<Tuple<List<Guid>, List<Guid>>> RefreshChannelsInternal(ILiveTvService service, IProgress<double> progress, CancellationToken cancellationToken)
    {
        ...
        numComplete = 0;
        var programIds = new ConcurrentBag<Guid>();
        var channels = new ConcurrentBag<Guid>();
        ...
        // Producer-consumer: producers fetch programs in parallel; consumer writes to DB serially.
        var pipe = Channel.CreateUnbounded<(LiveTvChannel Channel, List<LiveTvProgram> NewPrograms, List<LiveTvProgram> UpdatedPrograms, bool IsMovie, bool IsNews, bool IsSports, bool IsSeries, bool IsKids, List<Guid> ProgramIds)>();
        ...
        var producerTask = Parallel.ForEachAsync(
            ...
            async (currentChannel, ct) =>
            {
                ...
                try
                {
                    ...
                    var channelProgramIds = new List<Guid>();
                    ...
                    foreach (var program in channelPrograms)
                    {
                        ...
                        channelProgramIds.Add(programItem.Id);
                    ...
                    await pipe.Writer.WriteAsync((currentChannel, newPrograms, updatedPrograms, isMovie, isNews, isSports, isSeries, isKids, channelProgramIds), ct).ConfigureAwait(false);
                }
                catch (OperationCanceledException)
                {
                    throw;
                ...
                double percent = completed / (double)allChannelsList.Count;
                progress.Report((85 * percent) + 15);
            });

        // Complete the writer when all producers are done (or faulted).
        _ = producerTask.ContinueWith(_ => pipe.Writer.Complete(), TaskScheduler.Default);

        // Consumer: serially write to DB.
        await foreach (var (currentChannel, newPrograms, updatedPrograms, isMovie, isNews, isSports, isSeries, isKids, channelProgramIds) in pipe.Reader.ReadAllAsync(cancellationToken).ConfigureAwait(false))
        {
            _logger.LogDebug(
            ...
            foreach (var id in channelProgramIds)
            {
                programIds.Add(id);
            }
        }
        ...
        progress.Report(100);
        return new Tuple<List<Guid>, List<Guid>>(channels.ToList(), programIds.ToList());
    }
