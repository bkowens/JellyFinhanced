...
public static class JellyfinQueryHelperExtensions
{
    ...
    /// <summary>
    /// Builds an optimised query excluding items where a property matches any value in the list.
    /// This is the opposite of WhereOneOrMany - it filters OUT items that match.
    /// </summary>
    /// <typeparam name="TEntity">The entity.</typeparam>
    /// <typeparam name="TProperty">The property type to compare.</typeparam>
    /// <param name="query">The source query.</param>
    /// <param name="noneOf">The list of values to exclude.</param>
    /// <param name="property">Property expression.</param>
    /// <returns>A Query.</returns>
    public static IQueryable<TEntity> WhereNoneOf<TEntity, TProperty>(this IQueryable<TEntity> query, IList<TProperty> noneOf, Expression<Func<TEntity, TProperty>> property)
    {
        return query.Where(NoneOfExpressionBuilder(noneOf, property));
    }

    /// <summary>
    /// Builds an optimised query expression excluding items where a property matches any value in the list.
    /// </summary>
    /// <typeparam name="TEntity">The entity.</typeparam>
    /// <typeparam name="TProperty">The property type to compare.</typeparam>
    /// <param name="noneOf">The list of values to exclude.</param>
    ...
    public static Expression<Func<TEntity, bool>> NoneOfExpressionBuilder<TEntity, TProperty>(this IList<TProperty> noneOf, Expression<Func<TEntity, TProperty>> property)
    {
        ...
        if (noneOf.Count == 0)
        {
            // No exclusions - return always true
        ...
        if (noneOf.Count == 1)
        {
            ...
            {
                return Expression.Lambda<Func<TEntity, bool>>(Expression.NotEqual(property.Body, Expression.Constant(value, typeof(TProperty))), parameter);
            }
            else
            {
                return Expression.Lambda<Func<TEntity, bool>>(Expression.Not(Expression.ReferenceEqual(property.Body, Expression.Constant(value, typeof(TProperty)))), parameter);
            }
        }

        // For multiple values, build: (prop != val1 AND prop != val2 AND prop != val3 ...)
        // This avoids using Contains() which MySQL provider doesn't handle well
        Expression? combinedExpression = null;
        foreach (var value in noneOf)
        {
            Expression notEqual;
            if (typeof(TProperty).IsValueType)
            {
                notEqual = Expression.NotEqual(property.Body, Expression.Constant(value, typeof(TProperty)));
            }
            else
            {
                notEqual = Expression.Not(Expression.ReferenceEqual(property.Body, Expression.Constant(value, typeof(TProperty))));
            }

            combinedExpression = combinedExpression is null
                ? notEqual
                : Expression.AndAlso(combinedExpression, notEqual);
        }
    ...
    public static Expression<Func<TEntity, bool>> OneOrManyExpressionBuilder<TEntity, TProperty>(this IList<TProperty> oneOf, Expression<Func<TEntity, TProperty>> property)
    {
        ...
        // Handle empty list case - nothing matches an empty list
        if (oneOf.Count == 0)
        {
        ...
        if (oneOf.Count == 1)
        {
            ...
            if (typeof(TProperty).IsValueType)
            {
                return Expression.Lambda<Func<TEntity, bool>>(Expression.Equal(property.Body, Expression.Constant(value, typeof(TProperty))), parameter);
            }
            else
            {
                return Expression.Lambda<Func<TEntity, bool>>(Expression.ReferenceEqual(property.Body, Expression.Constant(value, typeof(TProperty))), parameter);
            }
        }
        ...
        // Always use constant embedding to generate IN(val1, val2, ...) SQL.
        // The EF.Parameter() path for parameterized primitive collections is not supported by all providers (e.g. Pomelo MySQL).
