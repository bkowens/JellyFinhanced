...
public class ArtistsController : BaseJellyfinApiController
{
    ...
    public async Task<ActionResult<QueryResult<BaseItemDto>>> GetArtists(
        ...
        [FromQuery] bool enableTotalRecordCount = true)
    {
        ...
        var dtos = await Task.WhenAll(result.Items.Select(async i =>
        {
            var (baseItem, itemCounts) = i;
            var dto = await _dtoService.GetItemByNameDtoAsync(baseItem, dtoOptions, null, user).ConfigureAwait(false);
    ...
    [ProducesResponseType(StatusCodes.Status200OK)]
    public async Task<ActionResult<QueryResult<BaseItemDto>>> GetAlbumArtists(
        [FromQuery] double? minCommunityRating,
        [FromQuery] int? startIndex,
        [FromQuery] int? limit,
        [FromQuery] string? searchTerm,
