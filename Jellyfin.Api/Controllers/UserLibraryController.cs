...
public class UserLibraryController : BaseJellyfinApiController
{
    ...
    [Obsolete("Kept for backwards compatibility")]
    [ApiExplorerSettings(IgnoreApi = true)]
    public Task<ActionResult<BaseItemDto>> GetRootFolderLegacy(
        [FromRoute, Required] Guid userId)
        ...
        [FromRoute, Required] Guid itemId)
    {
        ...
        var dtoOptions = new DtoOptions();
        var dtos = await Task.WhenAll(items.Select(i => _dtoService.GetBaseItemDtoAsync(i, dtoOptions, user))).ConfigureAwait(false);
        ...
        [FromRoute, Required] Guid itemId)
    {
        ...
        if (item is IHasTrailers hasTrailers)
        {
            ...
            return Ok((await _dtoService.GetBaseItemDtosAsync(trailers, dtoOptions, user, item).ConfigureAwait(false)).AsEnumerable());
        }

        return Ok((IEnumerable<BaseItemDto>)await Task.WhenAll(item.GetExtras()
            .Where(e => e.ExtraType == ExtraType.Trailer)
            .Select(i => _dtoService.GetBaseItemDtoAsync(i, dtoOptions, user, item))).ConfigureAwait(false));
    }
    ...
    [Obsolete("Kept for backwards compatibility")]
    [ApiExplorerSettings(IgnoreApi = true)]
    public Task<ActionResult<IEnumerable<BaseItemDto>>> GetLocalTrailersLegacy(
        [FromRoute, Required] Guid userId,
        [FromRoute, Required] Guid itemId)
        ...
        [FromRoute, Required] Guid itemId)
    {
        ...
        return Ok((IEnumerable<BaseItemDto>)await Task.WhenAll(item
            .GetExtras()
    ...
    [Obsolete("Kept for backwards compatibility")]
    [ApiExplorerSettings(IgnoreApi = true)]
    public Task<ActionResult<IEnumerable<BaseItemDto>>> GetSpecialFeaturesLegacy(
        [FromRoute, Required] Guid userId,
        [FromRoute, Required] Guid itemId)
        ...
        [FromQuery] bool groupItems = true)
    {
        ...
        var dtos = await Task.WhenAll(list.Select(async i =>
        {
            var item = i.Item2[0];
            var childCount = 0;
            ...
            var dto = await _dtoService.GetBaseItemDtoAsync(item, dtoOptions, user).ConfigureAwait(false);
        ...
        return Ok((IEnumerable<BaseItemDto>)dtos);
    }
