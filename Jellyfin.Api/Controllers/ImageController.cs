...
public class ImageController : BaseJellyfinApiController
{
        ...
        IDictionary<string, string> headers)
    {
        ...
        else
        {
            ...
            // Generate ETag from the modified date (strong ETag when cacheDuration is set, weak otherwise)
            var etagValue = cacheDuration.HasValue
                ? $"\"{dateImageModified.Ticks:x}\""
                : $"W/\"{dateImageModified.Ticks:x}\"";
            Response.Headers.ETag = etagValue;

            // Check If-None-Match for ETag-based conditional requests
            var ifNoneMatch = Request.Headers.IfNoneMatch.ToString();
            if (!string.IsNullOrEmpty(ifNoneMatch) && (ifNoneMatch == etagValue || ifNoneMatch == "*"))
            {
