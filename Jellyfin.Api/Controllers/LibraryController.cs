...
public class LibraryController : BaseJellyfinApiController
{
        ...
        [FromQuery, ModelBinder(typeof(CommaDelimitedCollectionModelBinder))] SortOrder[]? sortOrder = null)
    {
        ...
        var items = await Task.WhenAll(themeItems
            .Select(i => _dtoService.GetBaseItemDtoAsync(i, dtoOptions, user, item)))
    ...
    public async Task<ActionResult<AllThemeMediaResult>> GetThemeMedia(
        ...
        [FromQuery, ModelBinder(typeof(CommaDelimitedCollectionModelBinder))] SortOrder[]? sortOrder = null)
    {
        var themeSongs = await GetThemeSongs(
            itemId,
            userId,
            inheritFromParent,
            sortBy,
        ...
        var themeVideos = await GetThemeVideos(
            itemId,
            userId,
            inheritFromParent,
            sortBy,
