...
public class VideosController : BaseJellyfinApiController
{
    ...
    public async Task<ActionResult<QueryResult<BaseItemDto>>> GetAdditionalPart([FromRoute, Required] Guid itemId, [FromQuery] Guid? userId)
    {
        ...
        {
            var parts = video.GetAdditionalParts();
            var dtoTasks = parts.Select(i => _dtoService.GetBaseItemDtoAsync(i, dtoOptions, user, video));
            items = await Task.WhenAll(dtoTasks).ConfigureAwait(false);
        }
        else
        {
