...
public class MoviesController : BaseJellyfinApiController
{
        ...
        [FromQuery] int itemLimit = 8)
    {
        ...
        var similarToRecentlyPlayed = (await GetSimilarTo(user, recentlyPlayedMovies, itemLimit, dtoOptions, RecommendationType.SimilarToRecentlyPlayed).ConfigureAwait(false)).GetEnumerator();
        var similarToLiked = (await GetSimilarTo(user, likedMovies, itemLimit, dtoOptions, RecommendationType.SimilarToLikedItem).ConfigureAwait(false)).GetEnumerator();

        var hasDirectorFromRecentlyPlayed = (await GetWithDirector(user, recentDirectors, itemLimit, dtoOptions, RecommendationType.HasDirectorFromRecentlyPlayed).ConfigureAwait(false)).GetEnumerator();
        var hasActorFromRecentlyPlayed = (await GetWithActor(user, recentActors, itemLimit, dtoOptions, RecommendationType.HasActorFromRecentlyPlayed).ConfigureAwait(false)).GetEnumerator();
    ...
    private async Task<List<RecommendationDto>> GetWithDirector(
        User? user,
        ...
        RecommendationType type)
    {
        ...
        foreach (var name in names)
        {
            ...
            if (items.Count > 0)
            {
                var returnItems = await _dtoService.GetBaseItemDtosAsync(items, dtoOptions, user).ConfigureAwait(false);
    ...
    private async Task<List<RecommendationDto>> GetWithActor(User? user, IEnumerable<string> names, int itemLimit, DtoOptions dtoOptions, RecommendationType type)
    {
        var result = new List<RecommendationDto>();
        var itemTypes = new List<BaseItemKind> { BaseItemKind.Movie };
        if (_serverConfigurationManager.Configuration.EnableExternalContentInSuggestions)
    ...
    private async Task<List<RecommendationDto>> GetSimilarTo(User? user, IEnumerable<BaseItem> baselineItems, int itemLimit, DtoOptions dtoOptions, RecommendationType type)
    {
        var result = new List<RecommendationDto>();
        var itemTypes = new List<BaseItemKind> { BaseItemKind.Movie };
        if (_serverConfigurationManager.Configuration.EnableExternalContentInSuggestions)
        ...
        foreach (var item in baselineItems)
        {
            ...
            if (similar.Count > 0)
            {
                var returnItems = await _dtoService.GetBaseItemDtosAsync(similar, dtoOptions, user).ConfigureAwait(false);
