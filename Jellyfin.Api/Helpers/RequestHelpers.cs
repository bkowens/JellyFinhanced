...
public static class RequestHelpers
{
    ...
    internal static async Task<QueryResult<BaseItemDto>> CreateQueryResultAsync(
        ...
        User? user)
    {
        var dtos = await Task.WhenAll(result.Items.Select(async i =>
        {
            ...
            var dto = await dtoService.GetItemByNameDtoAsync(baseItem, dtoOptions, null, user).ConfigureAwait(false);
