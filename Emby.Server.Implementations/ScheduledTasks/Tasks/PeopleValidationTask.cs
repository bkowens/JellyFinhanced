            ...
            try
            {
                do
                {
                    ...
                    for (int i = 0; i < itemCounter; i++)
                    {
                        ...
                        // Update PeopleBaseItemMap records to point to the reference person
                        // Using manual update for MySQL provider compatibility (ExecuteUpdate not supported)
                        var mapsToUpdate = await context.PeopleBaseItemMap
                            .WhereOneOrMany(dups, e => e.PeopleId)
                            .ToListAsync(cancellationToken)
                            .ConfigureAwait(false);
                        foreach (var map in mapsToUpdate)
                        {
                            map.PeopleId = reference;
                        }

                        // Delete duplicate people using RemoveRange for MySQL compatibility
                        // (ExecuteDelete not supported)
                        var peoplesToDelete = await context.Peoples
                            .WhereOneOrMany(dups, e => e.Id)
                            .ToListAsync(cancellationToken)
                            .ConfigureAwait(false);
                        context.Peoples.RemoveRange(peoplesToDelete);
