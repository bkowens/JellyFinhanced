...
namespace Emby.Server.Implementations.Session
{
    ...
    public sealed class SessionManager : ISessionManager, IAsyncDisposable
    {
        ...
        private async Task UpdateNowPlayingItem(SessionInfo session, PlaybackProgressInfo info, BaseItem libraryItem, bool updateLastCheckInTime)
        {
            ...
            if (!info.ItemId.IsEmpty() && info.Item is null && libraryItem is not null)
            {
                ...
                if (current is null || !info.ItemId.Equals(current.Id))
                {
                    ...
                    info.Item = await GetItemInfoAsync(libraryItem, mediaSource).ConfigureAwait(false);
            ...
            if (nowPlayingQueue?.Length > 0 && !nowPlayingQueue.SequenceEqual(session.NowPlayingQueue))
            {
                ...
                session.NowPlayingQueueFullItems = await _dtoService.GetBaseItemDtosAsync(
                    _libraryManager.GetItemList(new InternalItemsQuery { ItemIds = itemIds }),
                    new DtoOptions(true)).ConfigureAwait(false);
            }
        }
        ...
        private async void CheckForIdlePlayback(object state)
        {
            ...
            {
                _logger.LogError(ex, "Unhandled error in CheckForIdlePlayback timer callback");
            }
        }
        ...
        private async void CheckForInactiveSteams(object state)
        {
            ...
            {
                _logger.LogError(ex, "Unhandled error in CheckForInactiveSteams timer callback");
            }
        }
        ...
        /// </summary>
        private async Task<BaseItemDto> GetItemInfoAsync(BaseItem item, MediaSourceInfo mediaSource)
        {
            ...
            var info = await _dtoService.GetBaseItemDtoAsync(item, dtoOptions).ConfigureAwait(false);
        ...
        /// <inheritdoc />
        public async Task ReportNowViewingItem(string sessionId, string itemId)
        {
            ...
            session.NowViewingItem = await GetItemInfoAsync(item, null).ConfigureAwait(false);
        }
