    ...
    : IItemRepository
{
    ...
    public void DeleteItem(params IReadOnlyList<Guid> ids)
    {
        ...
        var relatedItems = ids.SelectMany(f => TraverseHirachyDown(f, context)).ToList();
        ...
        // Use WhereOneOrMany for MySQL-compatible server-side filtering
        var userDataToDetach = context.UserData.WhereOneOrMany(relatedItems, e => e.ItemId).ToList();
        var userDataKeys = userDataToDetach.Select(e => new { e.UserId, e.CustomDataKey }).ToHashSet();
        var placeholderUserDataToDelete = context.UserData
            .Where(e => e.ItemId == PlaceholderId)
            .AsEnumerable()
            .Where(e => userDataKeys.Contains(new { e.UserId, e.CustomDataKey }))
            .ToList();
        context.UserData.RemoveRange(placeholderUserDataToDelete);

        // Detach all user watch data
        foreach (var userData in userDataToDetach)
        {
            userData.RetentionDate = date;
            userData.ItemId = PlaceholderId;
        }

        context.AncestorIds.RemoveRange(context.AncestorIds.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.AncestorIds.RemoveRange(context.AncestorIds.WhereOneOrMany(relatedItems, e => e.ParentItemId).ToList());
        context.AttachmentStreamInfos.RemoveRange(context.AttachmentStreamInfos.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.BaseItemImageInfos.RemoveRange(context.BaseItemImageInfos.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.BaseItemMetadataFields.RemoveRange(context.BaseItemMetadataFields.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.BaseItemProviders.RemoveRange(context.BaseItemProviders.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.BaseItemTrailerTypes.RemoveRange(context.BaseItemTrailerTypes.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.Chapters.RemoveRange(context.Chapters.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.CustomItemDisplayPreferences.RemoveRange(context.CustomItemDisplayPreferences.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.ItemDisplayPreferences.RemoveRange(context.ItemDisplayPreferences.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.ItemValuesMap.RemoveRange(context.ItemValuesMap.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.KeyframeData.RemoveRange(context.KeyframeData.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.MediaSegments.RemoveRange(context.MediaSegments.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.MediaStreamInfos.RemoveRange(context.MediaStreamInfos.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        var peopleIdsToCheck = context.PeopleBaseItemMap.WhereOneOrMany(relatedItems, e => e.ItemId).Select(f => f.PeopleId).Distinct().ToArray();
        context.PeopleBaseItemMap.RemoveRange(context.PeopleBaseItemMap.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        context.TrickplayInfos.RemoveRange(context.TrickplayInfos.WhereOneOrMany(relatedItems, e => e.ItemId).ToList());
        // Clean up orphaned item values and people
        context.SaveChanges();
        var orphanedItemValues = context.ItemValues.Where(e => e.BaseItemsMap!.Count == 0).ToList();
        context.ItemValues.RemoveRange(orphanedItemValues);
        var peopleIdsToCheckList = peopleIdsToCheck.ToList();
        var orphanedPeople = context.Peoples.WhereOneOrMany(peopleIdsToCheckList, e => e.Id).Where(e => e.BaseItems!.Count == 0).ToList();
        context.Peoples.RemoveRange(orphanedPeople);
        // Finally remove the base items
        context.BaseItems.RemoveRange(context.BaseItems.WhereOneOrMany(relatedItems, e => e.Id).ToList());
        context.SaveChanges();
        transaction.Commit();
    }

    /// <inheritdoc />
    public async Task DeleteItemAsync(params IReadOnlyList<Guid> ids)
    {
        if (ids is null || ids.Count == 0 || ids.Any(f => f.Equals(PlaceholderId)))
        {
            throw new ArgumentException("Guid can't be empty or the placeholder id.", nameof(ids));
        ...
        await using (context.ConfigureAwait(false))
        {
            ...
            await using (transaction.ConfigureAwait(false))
            {
                ...
                // Load user data to detach for the items being deleted
                var userDataToDetach = await context.UserData.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false);

                // Fix #2: avoid loading the entire placeholder UserData table into memory.
                // First pre-filter by userId server-side (small set), then match composite key client-side.
                var userIds = userDataToDetach.Select(e => e.UserId).Distinct().ToList();
                if (userIds.Count > 0)
                {
                    var userDataKeySet = userDataToDetach.Select(e => new { e.UserId, e.CustomDataKey }).ToHashSet();
                    var candidatePlaceholderData = await context.UserData
                        .Where(e => e.ItemId == PlaceholderId)
                        .WhereOneOrMany(userIds, e => e.UserId)
                        .ToListAsync().ConfigureAwait(false);
                    context.UserData.RemoveRange(
                        candidatePlaceholderData.Where(e => userDataKeySet.Contains(new { e.UserId, e.CustomDataKey })));
                }
                ...
                context.AncestorIds.RemoveRange(await context.AncestorIds.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.AncestorIds.RemoveRange(await context.AncestorIds.WhereOneOrMany(relatedItems, e => e.ParentItemId).ToListAsync().ConfigureAwait(false));
                context.AttachmentStreamInfos.RemoveRange(await context.AttachmentStreamInfos.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.BaseItemImageInfos.RemoveRange(await context.BaseItemImageInfos.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.BaseItemMetadataFields.RemoveRange(await context.BaseItemMetadataFields.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.BaseItemProviders.RemoveRange(await context.BaseItemProviders.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.BaseItemTrailerTypes.RemoveRange(await context.BaseItemTrailerTypes.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.Chapters.RemoveRange(await context.Chapters.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.CustomItemDisplayPreferences.RemoveRange(await context.CustomItemDisplayPreferences.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.ItemDisplayPreferences.RemoveRange(await context.ItemDisplayPreferences.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.ItemValuesMap.RemoveRange(await context.ItemValuesMap.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.KeyframeData.RemoveRange(await context.KeyframeData.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.MediaSegments.RemoveRange(await context.MediaSegments.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.MediaStreamInfos.RemoveRange(await context.MediaStreamInfos.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                var peopleIdsToCheck = await context.PeopleBaseItemMap.WhereOneOrMany(relatedItems, e => e.ItemId).Select(f => f.PeopleId).Distinct().ToArrayAsync().ConfigureAwait(false);
                context.PeopleBaseItemMap.RemoveRange(await context.PeopleBaseItemMap.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));
                context.TrickplayInfos.RemoveRange(await context.TrickplayInfos.WhereOneOrMany(relatedItems, e => e.ItemId).ToListAsync().ConfigureAwait(false));

                await context.SaveChangesAsync().ConfigureAwait(false);

                var orphanedItemValues = await context.ItemValues.Where(e => e.BaseItemsMap!.Count == 0).ToListAsync().ConfigureAwait(false);
                context.ItemValues.RemoveRange(orphanedItemValues);
                var peopleIdsToCheckList = peopleIdsToCheck.ToList();
                var orphanedPeople = await context.Peoples.WhereOneOrMany(peopleIdsToCheckList, e => e.Id).Where(e => e.BaseItems!.Count == 0).ToListAsync().ConfigureAwait(false);
                context.Peoples.RemoveRange(orphanedPeople);
                context.BaseItems.RemoveRange(await context.BaseItems.WhereOneOrMany(relatedItems, e => e.Id).ToListAsync().ConfigureAwait(false));
    ...
    public void UpdateInheritedValues()
    {
        ...
        var itemValuesToDelete = context.ItemValuesMap
            .Where(e => e.ItemValue.Type == ItemValueType.InheritedTags)
            .ToList();
        context.ItemValuesMap.RemoveRange(itemValuesToDelete);
    ...
    public IReadOnlyList<string> GetNextUpSeriesKeys(InternalItemsQuery filter, DateTime dateCutoff)
    {
        ...
        var query = context.BaseItems
            .AsNoTracking()
            .WhereOneOrMany(filter.TopParentIds, i => i.TopParentId!.Value)
...
#pragma warning disable CA1307 // Specify StringComparison for clarity
    ...
    public async Task SaveImagesAsync(BaseItemDto item, CancellationToken cancellationToken = default)
    {
        ...
        await using (context.ConfigureAwait(false))
        {
            ...
            // Use RemoveRange for MySQL provider compatibility (ExecuteDelete not supported)
            var imagesToDelete = await context.BaseItemImageInfos
                .Where(e => e.ItemId == item.Id)
                .ToListAsync(cancellationToken)
                .ConfigureAwait(false);
            context.BaseItemImageInfos.RemoveRange(imagesToDelete);
    ...
    public void UpdateOrInsertItems(IReadOnlyList<BaseItemDto> items, CancellationToken cancellationToken)
    {
        ...
        var ids = tuples.Select(f => f.Item.Id).ToHashSet();
        var idsList = ids.ToList();
        var existingItems = context.BaseItems
            .WhereOneOrMany(idsList, e => e.Id)
            .Select(e => e.Id)
            .ToHashSet();

        var updatedEntities = new List<BaseItemEntity>();
        ...
        // Bulk-delete child records for all updated items in 3 queries instead of 6N individual SELECT+DELETE pairs
        if (updatedEntities.Count > 0)
        {
            var updatedIdsList = updatedEntities.Select(e => e.Id).ToList();

            context.BaseItemProviders.RemoveRange(
                context.BaseItemProviders.WhereOneOrMany(updatedIdsList, e => e.ItemId).ToList());

            context.BaseItemImageInfos.RemoveRange(
                context.BaseItemImageInfos.WhereOneOrMany(updatedIdsList, e => e.ItemId).ToList());

            context.BaseItemMetadataFields.RemoveRange(
                context.BaseItemMetadataFields.WhereOneOrMany(updatedIdsList, e => e.ItemId).ToList());
        ...
        // Use case-insensitive comparison to match MySQL's utf8mb4_unicode_ci collation
        var itemValueComparer = ItemValueTupleComparer.Instance;
        var allListedItemValues = itemValueMaps
            .SelectMany(f => f.Values)
            .Distinct(itemValueComparer)
            .ToArray();

        // Pre-compute the lookup keys for efficient filtering
        var itemValueKeys = allListedItemValues
            .Select(e => (e.MagicNumber, e.Value))
            .ToHashSet(itemValueComparer);

        // Pre-filter by type server-side, then do composite key match in memory on the smaller result
        var allItemTypesList = allListedItemValues.Select(e => e.MagicNumber).Distinct().ToList();
        var existingValues = context.ItemValues
            .WhereOneOrMany(allItemTypesList, iv => iv.Type)
            .AsEnumerable()
            .Where(e => itemValueKeys.Contains((e.Type, e.Value)))
            .ToArray();
        var missingItemValues = allListedItemValues.Except(existingValues.Select(f => (MagicNumber: f.Type, f.Value)), itemValueComparer).Select(f => new ItemValue()
        {
        ...
        var valueMap = itemValueMaps
            .Select(f => (f.Item, Values: f.Values.Select(e => itemValuesStore.First(g => string.Equals(g.Value, e.Value, StringComparison.OrdinalIgnoreCase) && g.Type == e.MagicNumber)).DistinctBy(e => e.ItemValueId).ToArray()))
            .ToArray();

        var mappedValues = context.ItemValuesMap
            .WhereOneOrMany(idsList, e => e.ItemId)
            .ToList();
        ...
        // Issue #3 fix: pre-batch ancestor queries — 2 queries total instead of 2 per item.
        var itemsWithAncestors = tuples.Where(t => t.Item.SupportsAncestors && t.AncestorIds != null).ToList();
        if (itemsWithAncestors.Count > 0)
        {
            var ancestorItemIds = itemsWithAncestors.Select(t => t.Item.Id).ToList();

            // Single query for all existing ancestor records across all items
            var allExistingAncestors = context.AncestorIds
                .WhereOneOrMany(ancestorItemIds, e => e.ItemId)
                .ToList()
                .GroupBy(e => e.ItemId)
                .ToDictionary(g => g.Key, g => g.ToList());

            // Single query to validate all needed ancestor IDs across all items
            var allAncestorIdsNeeded = itemsWithAncestors.SelectMany(t => t.AncestorIds!).Distinct().ToList();
            var validAncestorIdSet = allAncestorIdsNeeded.Count > 0
                ? context.BaseItems.WhereOneOrMany(allAncestorIdsNeeded, e => e.Id).Select(f => f.Id).ToHashSet()
                : new HashSet<Guid>();

            foreach (var item in itemsWithAncestors)
            {
                var existingAncestorIds = allExistingAncestors.TryGetValue(item.Item.Id, out var existing)
                    ? existing
                    : new List<AncestorId>();

                foreach (var ancestorId in item.AncestorIds!.Where(id => validAncestorIdSet.Contains(id)))
                {
    ...
    public async Task ReattachUserDataAsync(BaseItemDto item, CancellationToken cancellationToken)
    {
        ...
        await using (dbContext.ConfigureAwait(false))
        {
            ...
            await using (transaction.ConfigureAwait(false))
            {
                var userKeys = item.GetUserDataKeys().ToHashSet();
                var retentionDate = (DateTime?)null;

                // Load and update in memory for MySQL compatibility with Contains()
                var userDataToUpdate = await dbContext.UserData
                    .Where(e => e.ItemId == PlaceholderId)
                    .ToListAsync(cancellationToken)
                    .ConfigureAwait(false);

                foreach (var userData in userDataToUpdate.Where(e => userKeys.Contains(e.CustomDataKey)))
                {
    ...
    public BaseItemEntity Map(BaseItemDto dto)
    {
        ...
        if (dto is Video video)
        {
            ...
            entity.VideoType = video.VideoType == MediaBrowser.Model.Entities.VideoType.VideoFile ? null : video.VideoType.ToString();
            entity.Is3D = video.Video3DFormat.HasValue ? true : null;
            entity.IsPlaceHolder = video.IsPlaceHolder ? true : null;
        }

        if (dto is Series series)
        {
            entity.SeriesStatus = series.Status?.ToString();
        }
        ...
        {
            entity.Images = dto.ImageInfos.Select(f => Map(dto.Id, f)).ToList();
        }
    ...
    private string[] GetItemValueNames(IReadOnlyList<ItemValueType> itemValueTypes, IReadOnlyList<string> withItemTypes, IReadOnlyList<string> excludeItemTypes)
    {
        ...
        var itemValueTypesList = itemValueTypes.Select(w => (ItemValueType)w).ToList();

        // Query ItemValues directly by type - server-side filter, no full table load
        IQueryable<ItemValue> query = context.ItemValues
            .AsNoTracking()
            .WhereOneOrMany(itemValueTypesList, iv => iv.Type);

        if (withItemTypes.Count > 0)
        {
            var withTypesList = withItemTypes.ToList();
            query = query.Where(iv =>
                iv.BaseItemsMap!.Any(m => withTypesList.Contains(m.Item.Type)));
        }

        if (excludeItemTypes.Count > 0)
        {
            var excludeTypesList = excludeItemTypes.ToList();
            query = query.Where(iv =>
                !iv.BaseItemsMap!.Any(m => excludeTypesList.Contains(m.Item.Type)));
        }
    ...
    private QueryResult<(BaseItemDto Item, ItemCounts? ItemCounts)> GetItemValues(InternalItemsQuery filter, IReadOnlyList<ItemValueType> itemValueTypes, string returnType)
    {
        ...
        if (filter.IncludeItemTypes.Length > 0)
        {
            ...
            // Execute a single GroupBy query instead of 7 individual COUNT queries
            var typeCounts = itemCountQuery!
                .GroupBy(f => f.Type)
                .Select(g => new { Type = g.Key, Count = g.Count() })
                .ToDictionary(x => x.Type, x => x.Count);

            var itemCountResult = new ItemCounts
            {
                SeriesCount = typeCounts.GetValueOrDefault(seriesTypeName),
                EpisodeCount = typeCounts.GetValueOrDefault(episodeTypeName),
                MovieCount = typeCounts.GetValueOrDefault(movieTypeName),
                AlbumCount = typeCounts.GetValueOrDefault(musicAlbumTypeName),
                ArtistCount = typeCounts.GetValueOrDefault(musicArtistTypeName),
                SongCount = typeCounts.GetValueOrDefault(audioTypeName),
                TrailerCount = typeCounts.GetValueOrDefault(trailerTypeName),
            };
            ...
            [
                .. query
                    ...
                    .Select(e => (Item: DeserializeBaseItem(e, filter.SkipDeserialization), ItemCounts: (ItemCounts?)itemCountResult))
        ...
        InternalItemsQuery filter)
    {
        ...
        if (filter.IncludeItemTypes.Length == 0)
        {
            ...
            else if (excludeTypes.Length > 1)
            {
                ...
                // Use WhereNoneOf for MySQL provider compatibility (avoids Contains() which MySQL doesn't support)
                baseQuery = baseQuery.WhereNoneOf(excludeTypeName, e => e.Type);
            }
        }
        else
        {
        ...
        {
            // Use WhereOneOrMany for MySQL provider compatibility
            var channelIds = filter.ChannelIds.ToList();
            baseQuery = baseQuery.Where(e => e.ChannelId != null).WhereOneOrMany(channelIds, e => e.ChannelId!.Value);
        }
        ...
        if (filter.ContributingArtistIds.Length > 0)
        {
            var contributingNames = context.BaseItems
                .WhereOneOrMany(filter.ContributingArtistIds, b => b.Id)
        ...
        {
            baseQuery = baseQuery.WhereOneOrMany(filter.OfficialRatings, e => e.OfficialRating);
        }
        ...
        {
            baseQuery = baseQuery.WhereNoneOf(filter.ExcludeItemIds, e => e.Id);
        }

        if (filter.ExcludeProviderIds is not null && filter.ExcludeProviderIds.Count > 0)
        {
            foreach (var kvp in filter.ExcludeProviderIds)
            {
                var provKey = kvp.Key;
                var provVal = kvp.Value;
                baseQuery = baseQuery.Where(e => !e.Provider!.Any(f => f.ProviderId == provKey && f.ProviderValue == provVal));
            }
        }
        ...
        if (filter.HasAnyProviderId is not null && filter.HasAnyProviderId.Count > 0)
        {
            ...
            {
                var providerFilter = includeAny.OneOrManyExpressionBuilder<BaseItemProvider, string>(f => f.ProviderId);
                baseQuery = baseQuery.Where(e => e.Provider!.AsQueryable().Any(providerFilter));
            }

            var includePairs = filter.HasAnyProviderId.Where(e => !string.IsNullOrEmpty(e.Value)).ToArray();
            if (includePairs.Length > 0)
            {
                // Build OR predicate: provider matches any of the key:value pairs
                var param = Expression.Parameter(typeof(BaseItemProvider), "f");
                var providerIdProp = Expression.Property(param, nameof(BaseItemProvider.ProviderId));
                var providerValueProp = Expression.Property(param, nameof(BaseItemProvider.ProviderValue));

                Expression? combined = null;
                foreach (var pair in includePairs)
                {
                    var keyMatch = Expression.Equal(providerIdProp, Expression.Constant(pair.Key));
                    var valMatch = Expression.Equal(providerValueProp, Expression.Constant(pair.Value));
                    var pairMatch = Expression.AndAlso(keyMatch, valMatch);
                    combined = combined is null ? pairMatch : Expression.OrElse(combined, pairMatch);
                }

                var providerPredicate = Expression.Lambda<Func<BaseItemProvider, bool>>(combined!, param);
                baseQuery = baseQuery.Where(e => e.Provider!.AsQueryable().Any(providerPredicate));
            }
        }
        ...
        if (queryTopParentIds.Length > 0)
        {
            ...
            {
                // Build expression for MySQL provider compatibility (avoids Contains() which isn't supported)
                var typeFilter = includedItemByNameTypes.ToList().OneOrManyExpressionBuilder<BaseItemEntity, string>(e => e.Type);
                var topParentFilter = queryTopParentIds.OneOrManyExpressionBuilder<BaseItemEntity, Guid>(e => e.TopParentId!.Value);
                baseQuery = baseQuery.Where(typeFilter.Or(topParentFilter));
            }
            else
            {
        ...
        {
            var seriesStatusList = filter.SeriesStatuses.Select(e => e.ToString()).ToList();
            baseQuery = baseQuery.WhereOneOrMany(seriesStatusList, e => e.SeriesStatus!);
        }
        ...
        {
            var videoTypeList = filter.VideoTypes.Select(e => e.ToString()).ToList();
            baseQuery = baseQuery.WhereOneOrMany(videoTypeList, e => e.VideoType!);
        }

        if (filter.Is3D.HasValue)
        {
            baseQuery = filter.Is3D.Value
                ? baseQuery.Where(e => e.Is3D == true)
                : baseQuery.Where(e => e.Is3D != true);
        }
        ...
        if (filter.IsPlaceHolder.HasValue)
        {
            baseQuery = filter.IsPlaceHolder.Value
                ? baseQuery.Where(e => e.IsPlaceHolder == true)
                : baseQuery.Where(e => e.IsPlaceHolder != true);
        }
    ...
    {
        // Use a recursive CTE to collect the full subtree in a single database round trip
        // instead of the previous BFS loop that issued one query per level of tree depth.
        var allDescendants = dbContext.Database.SqlQuery<Guid>(
            $"""
            WITH RECURSIVE hierarchy AS (
                SELECT Id FROM BaseItems WHERE Id = {parentId}
                UNION ALL
                SELECT b.Id FROM BaseItems b
                INNER JOIN hierarchy h ON b.ParentId = h.Id
            )
        ...
        // Apply the optional filter over the pre-collected descendant set.
        // This costs a second query but avoids BFS for trees of any depth.
        var descendantList = allDescendants.ToList();
        var filtered = dbContext.BaseItems
            .WhereOneOrMany(descendantList, e => e.Id)
            .Where(filter)
            .Select(e => e.Id)
            .ToHashSet();

        // Root is always part of the result regardless of the filter
    ...
    /// <summary>
    /// Case-insensitive equality comparer for (ItemValueType, string) tuples.
    /// Matches MySQL's utf8mb4_unicode_ci collation behavior for the IX_ItemValues_Type_Value unique index.
    /// </summary>
    private sealed class ItemValueTupleComparer : IEqualityComparer<(ItemValueType MagicNumber, string Value)>
    {
        public static readonly ItemValueTupleComparer Instance = new();

        public bool Equals((ItemValueType MagicNumber, string Value) x, (ItemValueType MagicNumber, string Value) y)
            => x.MagicNumber == y.MagicNumber && string.Equals(x.Value, y.Value, StringComparison.OrdinalIgnoreCase);

        public int GetHashCode((ItemValueType MagicNumber, string Value) obj)
            => HashCode.Combine(obj.MagicNumber, StringComparer.OrdinalIgnoreCase.GetHashCode(obj.Value));
    }
}
