...
#pragma warning disable CS0618 // Type or member is obsolete
        if (typeof(IMigrationRoutine).IsAssignableFrom(MigrationType))
        {
            ...
            {
                // Do not dispose the child provider: it borrows singleton instances
                // (e.g. ILoggerFactory) from the parent via factory delegates. Disposing
                // the child cascades and disposes those parent-owned singletons, breaking
                // subsequent migrations that rely on them.
